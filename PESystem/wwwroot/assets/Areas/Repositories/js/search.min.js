async function searchSerialNumbers(){const t=document.getElementById("sn-input")?.value||"",n=t.split("\n").map(n=>n.trim().toUpperCase()).filter(n=>n&&/^[A-Za-z0-9-]+$/.test(n));if(n.length===0){showError("Vui lòng nhập ít nhất một Serial Number hợp lệ!");return}try{const i=await fetch("https://pe-vnmbd-nvidia-cns.myfiinet.com/api/Search/SearchProductsBySN",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!i.ok)throw new Error("Không thể tải dữ liệu từ Server");const t=await i.json();console.log(t.success,"hihihihi");t.success?(searchResultsSN=t.results||[],renderTable(searchResultsSN,"results-body"),displayTotalResults(t.totalFound||0),document.getElementById("search-results").style.display="block",document.getElementById("export-sn-excel-btn").style.display="inline-block",document.getElementById("export-excel-btn").style.display="none",document.getElementById("button-action").classList.remove("hidden"),showSuccess(`Tìm thấy ${t.totalFound||0}/${t.totalNotFound+t.totalFound||0} SN`)):(showError(t.message||"Không tìm thấy kết quả!"),searchResultsSN=[],renderTable([],"results-body"))}catch(i){showError("Lỗi khi tìm kiếm: "+i.message)}finally{hideSpinner()}}function renderTable(n,t){const i=document.getElementById(t);if(!i){console.error(`Phần tử #${t} không tồn tại!`);return}if(i.innerHTML="",!n||n.length===0){i.innerHTML="<tr><td colspan='22'>Không tìm thấy kết quả!<\/td><\/tr>";return}n.forEach(n=>{const t=`
            <tr>
