...
LEFT JOIN (
    SELECT SERIAL_NUMBER, TEST_TIME, TEST_CODE, TEST_GROUP, ERROR_ITEM_CODE
    FROM (
        SELECT 
            R.SERIAL_NUMBER, 
            R.TEST_TIME, 
            ...
            R.TEST_GROUP, 
            R.ERROR_ITEM_CODE,
            ROW_NUMBER() OVER (PARTITION BY R.SERIAL_NUMBER ORDER BY R.TEST_TIME DESC) AS rn
...
-- Subquery for key parts
LEFT JOIN (
    SELECT 
        K.KEY_PART_SN, 
        MAX(K.SERIAL_NUMBER) AS PARENT_SN
...
LEFT JOIN (
    SELECT SERIAL_NUMBER, LATEST_TEST_TIME, LATEST_TEST_CODE, LATEST_TEST_GROUP, LATEST_ERROR_ITEM_CODE
    FROM (
        SELECT 
            R.SERIAL_NUMBER, 
            R.TEST_TIME AS LATEST_TEST_TIME, 
            R.TEST_CODE AS LATEST_TEST_CODE, 
            R.TEST_GROUP AS LATEST_TEST_GROUP, 
            R.ERROR_ITEM_CODE AS LATEST_ERROR_ITEM_CODE,
            ROW_NUMBER() OVER (PARTITION BY R.SERIAL_NUMBER ORDER BY R.TEST_TIME DESC) AS rn
        FROM SFISM4.R109 R
        WHERE R.TEST_TIME IS NOT NULL
    ) 
